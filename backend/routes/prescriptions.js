...
router.get('/', authenticate, async (req, res) => {
    try {
        ...
        const [countRows] = await pool.query(query.replace('SELECT p.*, c.name as customer_name', 'SELECT COUNT(*) as total'), params);
        const total = countRows[0].total;
        query += ' ORDER BY p.created_at DESC LIMIT ? OFFSET ?';
        const limitNum = parseInt(limit) || 20;
        const offsetNum = (parseInt(page) - 1) * limitNum;
        const [rows] = await pool.query(query, [...params, limitNum, offsetNum]);
        res.json({ success: true, data: rows, total, pages: Math.ceil(total / limitNum) });
